    ...
    **kwargs
    ...
    if pointers.get("pointer_to_mass") is not None:
        ...
        if to_logm == True and to_mass_hcorr == False:
            ...
            print(
                sim_name
                + ": uh oh, something's up with the mass (check your mass flags)"
            )
            mass = None
    ...
    if pointers["pointer_to_specdat"] == None:
        ...
        if posvel is not None:
            ...
            if len(d400) != len(posvel):
                print(
                    sim_name + ": phot data and position data are not the same length!"
                )
        else:
            ngals = None
...
def get_HAEW(wave, spec):
    ...
    spec_cont_Ha = spec1[bandw_Ha] + np.median(
        np.concatenate((spec_em[bandw_Ha_blueside], spec_em[bandw_Ha_redside]), axis=0)
    )

    HaEW = tsum(
        wave[bandw_Ha], np.divide((spec_cont_Ha - spec[bandw_Ha]), spec_cont_Ha)
    )
...
class Simulation(object):
    ...
    def convert_posvel_to_spherical(
        self, pos_type="galaxies", cartesian_origin=(0, 0, 0)
    ):
        ...
        if pos_type == "galaxies":
            ...
            self.obs.update(
                rr=np.sqrt(
                    (xx - cartesian_origin[0]) ** 2
                    + (yy - cartesian_origin[1]) ** 2
                    + (zz - cartesian_origin[2]) ** 2
                )
            )
            self.obs.update(
                theta=np.arccos((zz - cartesian_origin[2]) / self.obs["rr"])
            )
            self.obs.update(
                phi=np.arctan2(yy - cartesian_origin[1], xx - cartesian_origin[0])
            )
        elif pos_type == "host":
    ...
    def generate_massive_box(self, mass_threshold=10.39):
        ...
        for i in range(26):
            bounding_box = np.append(
                bounding_box,
                addon + offsets[i] * self.data["boxsize"] / self.data["little_h"],
                axis=0,
            )
        self.massive_box = bounding_box
        self.massive_mass = np.ravel(
            [self.data["mass"][self.data["mass"] > mass_threshold]] * 27
        )

    def calculate_dhost_true(
        self,
    ):
    ...
    def calculate_qfrac_true(self, mass_bins, dhost_bins):
        ...
        for i in range(len(mass_bins) - 1):
            for j in range(len(dhost_bins) - 1):
                n_all = len(
                    self.data["mass"][
                        (self.data["mass"] > mass_bins[i])
                        & (self.data["mass"] < mass_bins[i + 1])
                        & (self.dhost_true > dhost_bins[j])
                        & (self.dhost_true < dhost_bins[j + 1])
                    ]
                )
                n_q = len(
                    self.data["mass"][
                        ...
                        & (self.dhost_true > dhost_bins[j])
                        & (self.dhost_true < dhost_bins[j + 1])
                        & (self.data["d400"] > 0.6 + 0.1 * self.data["mass"])
                        & (self.data["haew"] < 2)
                    ]
                )
    ...
    def create_single_sightline(self, dist_away=5):
        ...
        self.convert_posvel_to_spherical(
            cartesian_origin=self.obs["origin"], pos_type="galaxies"
        )
        self.convert_posvel_to_spherical(
            cartesian_origin=self.obs["origin"], pos_type="host"
        )

        self.obs.update(
            rmag_app=self.data["rmag"] + 5 * np.log10(self.obs["rr"] * 10 ** 5)
        )
    ...
    def calculate_vmax(self, n_points=50):
        ...
        box_rr = np.ravel(
            np.sqrt(
                (xx_g - self.obs["origin"][0]) ** 2
                + (yy_g - self.obs["origin"][1]) ** 2
                + (zz_g - self.obs["origin"][2]) ** 2
            )
        )

        mag_lim = self.run_params["obs_mag_lim"]
        lim_dist = (10 ** ((mag_lim - self.data["rmag"]) / 5.0)) / 10 ** 5
    ...
    def calculate_dhost_proj(self):
        ...
        for i in range(self.data["ngals"]):
            ang_dist = (
                (self.obs["theta"][i] - self.obs["proj_host_pos"][1]) ** 2
                + (self.obs["phi"][i] - self.obs["proj_host_pos"][2]) ** 2
            ) ** 0.5
            possible_dhosts = (ang_dist * self.obs["rr"][i])[
                (ang_dist < 7 / self.obs["rr"][i])
    ...
    def add_noise_to_spectra(self):
        ...
        if (
            self.run_params["add_noise_to_spectra"] == True
            and self.run_params["add_gr_noise"] == False
        ):
            ...
            for i in range(self.data["ngals"]):
                if rmag_app[i] < self.run_params["obs_mag_lim"]:
                    if digi_rmag[i] == 0:
                        ...
                        new_digi = np.arange(1, len(rmag_bins))[rbins_mask][
                            np.abs(
                                digi_rmag[0]
                                - 1
                                - np.arange(1, len(rmag_bins))[rbins_mask]
                            ).argmin()
                        ]
                    else:
                    ...
                    noisy_spec[i] = np.random.normal(
                        loc=interp_spec, scale=interp_spec / binned_snr[new_digi - 1]
                    )
            ...
            self.run_params["add_noise_to_spectra"] == True
            and self.run_params["add_gr_noise"] == True
        ):
            rmag_app = self.obs["rmag_app"]
            ...
            for i in range(self.data["ngals"]):
                if rmag_app[i] < self.run_params["obs_mag_lim"]:
                    bin_id = (
                        np.sum(
                            (
                                (bin_center_points - (rmag_app[i], gr_color[i]))
                                / (0.5, 0.1)
                            )
                            ** 2,
                            axis=1,
    ...
    def calculate_qfrac_proj(self, mass_bins, dhost_bins):
        ...
        self.obs["qfrac_proj_novm"] = np.zeros(
            (len(mass_bins) - 1, len(dhost_bins) - 1, 2)
        )
        ...
        for j in range(len(mass_bins) - 1):
            for i in range(len(dhost_bins) - 1):
                self.obs["qfrac_proj_vmax"][j][i][0] = np.sum(
                    1
                    / vmax_weights[
                        (digi_mass == j + 1)
                        & (digi_dhost == i + 1)
                ...
                self.obs["qfrac_proj_vmax"][j][i][1] = np.sum(
                    1
                    / vmax_weights[
                        (digi_mass == j + 1)
                        & (digi_dhost == i + 1)
                ...
                self.obs["qfrac_proj_novm"][j][i][0] = len(
                    mass[
                        (digi_mass == j + 1)
                        & (digi_dhost == i + 1)
                        & (haew < 2)
                ...
                self.obs["qfrac_proj_novm"][j][i][1] = len(
                    mass[
                        (digi_mass == j + 1)
                        & (digi_dhost == i + 1)
                        & (rmag_app < mag_lim)
    ...
    def create_multiple_sightlines(self, mass_bins, dhost_bins):
        ...
        self.quenched_fractions = np.zeros(
            (n_sightlines, len(mass_bins) - 1, len(dhost_bins) - 1, 2)
        )
    ...
    def calculate_galaxy_sizes(self):
        ...
        for i in range(len(Mstar)):
            if Mstar[i] > 11.0:
                Msize[i] = np.abs(
                    np.random.normal(loc=inter_size(11.0), scale=1.0) + sigma[i]
                )
    ...
    def calculate_surface_brightness(self):
        ...
        half_light_surface_brightness = half_light_mag + 2.5 * np.log10(
            size_arcsec ** 2
        )
